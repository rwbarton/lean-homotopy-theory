...
assume ⟨⟨r, h, ⟨H, Hrel⟩⟩⟩, begin
  refine ⟨⟨po.induced (f ∘ r) (𝟙 a') (by rw [←associativity, h]; simp), by simp, _⟩⟩,
  ...
  { unfold homotopy.is_rel at Hrel, rw [←associativity, Hrel, p_nat_assoc],
    have : f' ∘ (j ∘ r ∘ j ∘ p @> a) = (f' ∘ j) ∘ (r ∘ j) ∘ p @> a, by simp,
    rw [this, po.commutes, h], dsimp, simp },
  -- The homotopy defined in this way starts at j' ∘ r' : x' → x':
  { apply po.uniqueness; rw i_nat_assoc; conv { to_rhs, rw ←associativity }; simp;
      rw ←associativity,
    { rw [H.Hi₀, associativity, po.commutes] },
    { rw [pi_components], simp } },
  -- ... and ends at 𝟙 x':
  { apply po.uniqueness; rw i_nat_assoc; simp; rw [←associativity],
    { rw H.Hi₁; simp }, { rw [pi_components], simp } },
  -- ... and is rel j:
  { unfold homotopy.is_rel,
    simp, congr, rw [←associativity], dsimp, simp }
end
